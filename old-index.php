<html>
<head>
    <title>Cherry Blossom Development</title>
    <style>
        .palette span {
            padding: 10px;
        }
        .light-crimson { background-color: #F76F8E; }
        .granite-gray { background-color: #686868; }
        .rackley { background-color: #6A8EAE; }
        .pistacio { background-color: #95BF74; }
        .pale-robin-egg-blue { background-color: #99DDC8; }
    </style>
</head>
<body>
    <div id="header">
        <h2>Cherry Blossom Development</h2>
    </div>
    <div id="main-navigation">
        <ul>
            <li>Who are we?</li>
            <li>Mission Statement</li>
            <li>Projects</li>
        </ul>
    </div>
    <div id="main-content">
        <div id="like_button_container"></div>
        <img src="images/cherry-blossoms-tokyo-ueno-park.jpg"/>
    </div>
    <div id="footer">
        <div class="palette">
            <span class="light-crimson">Light Crimson</span>
            <span class="granite-gray">Granite Grey</span>
            <span class="rackley">Rackley</span>
            <span class="pistacio">Pistacio</span>
            <span class="pale-robin-egg-blue">Pale Robin Egg Blue</span>
        </div>
    </div>
    <!-- Non prod scripts
    <script src="https://unpkg.com/react@16/umd/react.development.js" crossorigin></script>
    <script src="https://unpkg.com/react-dom@16/umd/react-dom.development.js" crossorigin></script>
    <script src="like_button.js"></script>
    -->
    <!-- Prod Scripts -->
    <script src="https://unpkg.com/react@16/umd/react.production.min.js" crossorigin></script>
    <script src="https://unpkg.com/react-dom@16/umd/react-dom.production.min.js" crossorigin></script>
    <script src="like_button.min.js"></script>
</body>
</html>
